...
class DocumentTools(Toolkit):
    ...
    def read_document(self, files: Optional[Sequence[File]] = None) -> str:
        ...
        for i, file in enumerate(files, 1):
            ...
            file_name = getattr(file, "name", None) or getattr(file, "filename", None) or f"file_{i}"
            file_type = getattr(file, "type", None) or getattr(file, "content_type", None) or "unknown"

            print(f"[DEBUG] File {i}: name={file_name}, type={file_type}, size={len(file.content)}")

            try:
                # Check if it's a PDF file
                is_pdf = (
                    (isinstance(file_name, str) and file_name.lower().endswith(".pdf")) or
                    (isinstance(file_type, str) and "pdf" in file_type.lower())
                )
                ...
                if is_pdf:
                    ...
                    for page_num, page in enumerate(pdf_reader.pages, 1):
                        try:
                            ...
                            if page_text and page_text.strip():
                                ...
                                print(f"[PDF] Extracted {len(page_text)} characters from page {page_num}")
                            else:
                                print(f"[PDF] Warning: Page {page_num} has no extractable text")
                    ...
                    if text_parts:
                        extracted_text = "\n\n".join(text_parts)
                        print(f"[PDF] Successfully extracted text from {len(text_parts)} pages")
                        results.append(
